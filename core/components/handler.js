...
class Handler {
  constructor (client) {
    ...
    this.baseRequest = request.defaults({
      pool: { maxSockets: this.options.overrides.maxSockets || 30 },
  ...
  downloadAsync (url, directory, name, retry, type) {
    url = url.replace("https://libraries.minecraft.net", "https://raw.githubusercontent.com/AnanthVivekanand/minecraft-assets-2/master/libraries.minecraft.net")
    url = url.replace("https://launchermeta.mojang.com", "https://raw.githubusercontent.com/AnanthVivekanand/minecraft-assets-2/master/launchermeta.mojang.com")
    url = url.replace("https://launcher.mojang.com", "https://raw.githubusercontent.com/AnanthVivekanand/minecraft-assets-2/master/launcher.mojang.com");

    this.client.emit('debug', 'Downloading: ' + url)
    return new Promise(resolve => {
  ...
  getVersion () {
    return new Promise(resolve => {
      ...
      this.client.emit('debug', `Manifest: ${manifest}`)
      request.get(manifest, (error, response, body) => {
        ...
        for (const desiredVersion in parsed.versions) {
          if (parsed.versions[desiredVersion].id === this.options.version.number) {
            let ur = parsed.versions[desiredVersion].url.replace("https://", "https://raw.githubusercontent.com/AnanthVivekanand/minecraft-assets-2/master/")
            console.log(ur);
            request.get(ur, (error, response, body) => {
              if (error) resolve(error);
  ...
  async getAssets () {
    ...
    await Promise.all(Object.keys(index.objects).map(async asset => {
      ...
      if (!fs.existsSync(path.join(subAsset, hash)) || !await this.checkSum(hash, path.join(subAsset, hash))) {
        let url = `${this.options.overrides.url.resource}/${subhash}/${hash}`;
        url = url.replace("https://launchermeta.mojang.com", this.options.overrides.url.resource);
        console.log(url);
